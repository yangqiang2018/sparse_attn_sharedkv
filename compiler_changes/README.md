...
| 011 | [Ascend `gemm_v0_fixp` 加 `flush_last`/`do_fixpipe`：per-K-chunk 累加（内核驱动 GM→L1 链式切块 = `ComputeMm1` kL1 切分）](011-ascend-gemm-v0-fixp-kchunk.md) | `wip/gemm-v0-fixp-kchunk` `d1f9e516` | ⏳ 待 NPU 验证(SWA 5/5 + 回归)后合入 `ascendc_pto` |
