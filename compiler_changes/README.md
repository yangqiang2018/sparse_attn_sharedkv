...
| 012 | [Ascend `T.mma`/`T.copy(L0C→GM)` 接通 `unitFlag`（内核驱动 fixpipe∥mma 融合 = 环感知 gemm 拆分前置；非旧 012，旧 `dst_row_stride` 已否）](012-ascend-mma-fixpipe-unitflag.md) | ⏳ 待合入 | ⏳ 待合入 `ascendc_pto`（Layer ③ PV/QK 解构前置原语） |
