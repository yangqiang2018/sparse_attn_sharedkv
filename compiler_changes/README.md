...
| 012 | [Ascend `gemm_v0_fixp` 加 `dst_row_stride`：per-N-tile 输出写进宽 dst 的列段（内核驱动 `ComputeMm2` nL1 输出切分）](012-ascend-gemm-v0-fixp-dst-row-stride.md) | `wip/gemm-v0-fixp-dststride` `89a5d12c` | ⏳ 待 NPU 验证(SWA 5/5 + 回归)后合入 `ascendc_pto` |
