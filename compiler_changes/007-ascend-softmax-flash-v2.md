...
- **模板按需实例化**：`softmax_flash_v2` C++ 模板只在被发出的调用点实例化；无任何现存内核调用它，故
  现存算子编译产物字节级不变（与 004/005/006 同构）。`common.h` 显式加一行
  `#include "lib/activation/softmaxflashv2.h"` 保证 `SoftmaxFlashV2` 可见——该头有 include guard、
  `begin_pipe(V)`/`end_pipe` 自闭合，对不调用本原语的算子完全 inert。
- **API 已离线逐字核对**（克隆 `gitee.com/ascend/ascendc-api-adv` 比对 AscendC 源）：带
  `sharedTmpBuffer` 的 `SoftmaxFlashV2` 重载在 `lib/activation/softmaxflashv2.h:158`，10 个参数顺序
  （dst, expSum, max, src, expMax, inExpSum, inMax, **sharedTmpBuffer**, tiling, shapeInfo）、6 个模板
  布尔位、`SoftMaxShapeInfo{srcM, srcK(对齐 stride), oriSrcM, oriSrcK(有效列)}`、`SoftmaxConfig` 4-参
  构造、设备端 `__aicore__ SoftMaxFlashV2TilingFunc`、`tmp.GetSize()`（字节）全部与本模板逐字一致。
