...
### 5b. 运行期 `k_actual`(变长 K 收缩,= Ascend C `kSize=window`)

`gemm_v0_fixp` 增加一个运行期参数 `k_actual`(`<= K`):只把 K 维的前 `k_actual` 行载入 L0
并参与收缩。SWA 的 PV(`O = P @ V`,K=窗口维)传 `k_actual = winm`(实际窗口长度),与
Ascend C `ComputeMm2` 用 `kSize=actualSingleProcessSInnerSize`(精确窗口)逐指令一致。

**为什么必须**:`copy_pa` 只写 `kv_l1[0:win]`,pad 行 `[win:BI]` **不写也不清零**——在每核
**首任务**是未初始化 L1(可能是 NaN/inf 位型)。若 PV 按固定 `K=BI` 收缩,`p_l1[:,win:BI]=0`
(掩码)却仍要算 `0 × kv_l1[win:BI]`,`0×NaN=NaN`,污染该 (token,head) 行(实测 fast 测试
99.79% 正确、~0.21% 出 NaN,局部集中在早 token,正是此因)。用 `k_actual=winm` 让 pad 行
**根本不进 L0B、不参与累加**,从源头消除 `0×garbage`——这与"变长 N"语义等价,且就是参考的
做法,**不是清零兜底的绕行**。单 K-tile(`K<=128`)适用,`static_assert(K<=kL0Size)` 守卫;
`k_actual` 默认 `=K`(不传则全量收缩,旧行为不变)。
