# 009 · Ascend `gemm_v0_fixp` K-累加 + `n_actual` + `cl0_base`(QK 走统一 fixp 路径 = Ascend C `ComputeMm1`,与 PV 共享 cL0)

| | |
|---|---|
| **编译器仓库** | `yangqiang2018/tilelang-ascend-2` |
| **分支 / 提交** | `wip/gemm-v0-fixp-kaccum`（**独立基于 `ascendc_pto`**,含 001–008;`0b0f8994` K-累加+n_actual → `1c7c124a` tiny-tile 屏障修复 → `787c4ce5` 加 `cl0_base` 共享 cL0） |
| **改动文件** | `src/tl_templates/ascend/common.h`(`gemm_v0_fixp`)、`tilelang/language/ascend.py`(绑定加 `n_actual`、`cl0_base`)、`src/target/codegen_ascend.cc`(`GemmFixpOpCodegen` 多发两参)、`src/op/ascend.cc`(`ascend_gemm_v0_fixp` set_num_inputs 7→9) |
| **是否必须** | 是 —— 忠实复刻要求 QK 与 PV 走**同一套 matmul 结构 + 共享同一个 cL0TensorPingPong**(参考 `ComputeMm1`/`ComputeMm2` 共用一个 `cL0BufIter`),QK 不能再用 `gemm_v0` 留驻 + 单独拷、也不能用分开的 cL0 |
| **是否兼容** | 是 —— PV/现有 caller:`K≤128`、`n_actual=N`、`cl0_base=0` 全默认 → 逐字节不变;回归两 example PASS |
| **状态** | ⏳ 待 NPU 验证。调试史:首测因 tiny-tile 屏障用编译期 `nTile`(对 QK 小窗口漏屏障)**卡死** → `1c7c124a` 改运行期 `mmaN`;重测**仍卡死** → 怀疑 QK/PV 用分开 cL0 让 unitFlag 旋转不连续 → `787c4ce5` 加 `cl0_base` 让两者共享一个 cL0、连续旋转(= 参考)。重测中。 |
...
- **加 `uint32_t cl0_base = 0`(787c4ce5)**:让 QK 与 PV **共享一个** cL0 ping-pong——
  `c_base = ((cl0_base + cL0BufIter) & 1) * (M*nTile)`。QK 传 `cl0_base=0`(用 slot 0,1 个 N-tile)、
  PV 传 `cl0_base=1`(4 个 N-tile → slot 1,0,1,0),于是 unitFlag 的 mma→fixpipe 旋转在 QK 和 PV 之间
  是**一条连续序列**,正如参考用**同一个** `cL0BufIter` 横跨 `ComputeMm1`+`ComputeMm2`。之前用分开的
  acc_s/acc_o,两个 unitFlag gemm 之间旋转不连续——这是第二轮卡死的怀疑根因。默认 0 = 原独立行为,保兼容。

绑定/codegen/set_num_inputs 照 `n_actual`/`k_actual` 既有范式透传(尾随默认参,arg [7]=n_actual、[8]=cl0_base)。
