...
This is NOT a pytest -- run it directly on an Ascend NPU host:
    ...
    python sparse_flash_mla_perf.py
    python sparse_flash_mla_perf.py --scenarios csa_prefill
    python sparse_flash_mla_perf.py --warmup 10 --iters 50
    python sparse_flash_mla_perf.py --only tilelang   # one side only
...
DEFAULT_SCENARIOS = ["csa_prefill", "swa_prefill", "hca_prefill"]
...
def create_tensor_with_stride_padding(src_tensor, pad_len):
    ...
    storage_tensor = torch.full((physical_numel,), float("nan"), dtype=dtype, device=device)
...
def stage_on_npu(c, stride_pad=True):
    ...
    if stride_pad:
        ...
        inp["cmp_pa_pad_npu"] = create_tensor_with_stride_padding(dev(c["cmp_pa"]), 64) if c["cmp_pa"] is not None else None
    else:
...
def tilelang_sharedkv(inp, cfg):
    """Run ONLY the TileLang sharedkv op. The TileLang port does not consume the
    scheduler metadata (the kernel uses a static even split), so there is no
    separate metadata op to build or time on the TileLang side."""
...
def tilelang_once(inp, cfg):
    """Run only the TileLang sharedkv op; return (md_ms, sk_ms). TileLang has no
    separate scheduler-metadata op, so md_ms is always 0.0 -- the Ascend C side
    still pays its metadata cost, which keeps the comparison honest."""
    _sync()
    t0 = perf_counter()
    tilelang_sharedkv(inp, cfg)
    _sync()
    t1 = perf_counter()
    return 0.0, (t1 - t0) * 1e3
...
def _fmt_line(label, ac_ms, tl_ms):
    ...
    return f"  {label:<10}  AscendC={ac_ms:9.4f} ms   TileLang={tl_ms:9.4f} ms   TileLang perf = {pct:6.1f}% of Ascend C"
...
def run_scenario(name, dtype, warmup, iters, only, stride_pad):
    ...
    if "ac_md" in result and "tl_md" in result:
        ...
        print("  -- median latency (lower=better); TileLang perf = AscendC / TileLang x 100% --")
...
def print_summary(results, only):
    ...
    print(f"{'scenario':<14}{'metadata':>12}{'sharedkv':>12}{'overall':>12}   (perf% = AscendC / TileLang)")
...
def main():
    ...
    ap.add_argument("--warmup", type=int, default=5, help="untimed warm-up iterations (default 5)")
    ap.add_argument("--iters", type=int, default=30, help="timed iterations (default 30)")
    ap.add_argument(
        "--only",
        default="both",
    ...
    ap.add_argument(
        "--no-stride-pad",
        action="store_true",
        help="skip the Ascend C PA_ND stride padding (by default it is applied to match the reference flow)",
    )
    args = ap.parse_args()
    ...
    if not torch.npu.is_available():
        print("[fatal] torch.npu.is_available() == False; need an NPU.", file=sys.stderr)
        return 2
    if args.only in ("both", "ascendc"):
        try:
            ...
            print(
                f"[fatal] could not import custom_ops (Ascend C op): {exc!r}\nBuild/install the Ascend C operator, or use --only tilelang.",
                file=sys.stderr,
            )
            return 2
